...
class PythonMapper implements ITargetLanguageMapper {
	...
	private generateNode(
		...
		// bind context to keep signature
	...
	private generateProgram(
		...
		(node.children || []).forEach((childNode) =>
			this.generateNode(childNode, lines, level, loopIterators)
		);
	}
	...
	private generateForLoop(
		...
		const fromRaw = node.value?.from ?? 0;
		const step = node.value?.step ?? 1;
		const untilRaw = node.value?.until ?? 'LIMIT';

		const startExpr = this.normalizeOperand(fromRaw);
		const stopExpr = this.rangeStop(untilRaw, step);
		const stepExpr = step !== 1 ? `, ${step}` : '';
		lines.push(
			`${this.indent(level)}for ${iterator} in range(${startExpr}, ${stopExpr}${stepExpr}):`
		);
		loopIterators.add(iterator);
		(node.children || []).forEach((childNode) =>
			this.generateNode(childNode, lines, level + 1, loopIterators)
		);
	...
	private generateIfStatement(
		...
		const leftRaw = node.value?.left ?? 'cond_left';
	...
	private generatePrintStatement(
		...
		if (typeof value === 'string') {
			lines.push(`${this.indent(level)}print(${JSON.stringify(value)})`);
			return;
		}
		...
		lines.push(`${this.indent(level)}print(${JSON.stringify(text)}${argsExpr})`);
	}
	...
	private generateVarDeclaration(
		...
		if (init !== undefined) {
			...
			lines.push(`${this.indent(level)}${name} = ${initExpr}`);
		} else {
			lines.push(`${this.indent(level)}${name} = None`);
		}
	}
	...
	private rangeStop(untilRaw: any, step: number): string {
		if (typeof untilRaw === 'number') return String(untilRaw + 1);
		const untilExpr = this.normalizeOperand(untilRaw);
		const delta = step < 0 ? '- 1' : '+ 1';
		return `${untilExpr} ${delta}`;
	}
	...
	private normalizeIdentifier(raw: any): string {
		...
		const lowered = raw.replace(this.RE_NON_ALPHANUM, '_').replace(/_{2,}/g, '_').toLowerCase();
...
export const pythonMapper = new PythonMapper();
